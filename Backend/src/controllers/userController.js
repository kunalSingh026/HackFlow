    ...
    try {
        ...
        if (techStackTags) {
            ...
            parsedTechStack = Array.isArray(tempTechStack)
                ? tempTechStack 
                : typeof tempTechStack === 'string' ? tempTechStack.split(',').map(tag => tag.trim()).filter(Boolean) : req.user.techStackTags;
        }
        ...
        const updatedLinks = {
            githubUrl: incomingLinks?.githubUrl !== undefined ? incomingLinks.githubUrl : req.user.links?.githubUrl,
            linkedinUrl: incomingLinks?.linkedinUrl !== undefined ? incomingLinks.linkedinUrl : req.user.links?.linkedinUrl,
            leetcodeUrl: incomingLinks?.leetcodeUrl !== undefined ? incomingLinks.leetcodeUrl : req.user.links?.leetcodeUrl,
            codeforcesUrl: incomingLinks?.codeforcesUrl !== undefined ? incomingLinks.codeforcesUrl : req.user.links?.codeforcesUrl,
            portfolioUrl: incomingLinks?.portfolioUrl !== undefined ? incomingLinks.portfolioUrl : req.user.links?.portfolioUrl,
        };

        const updatedEducation = {
            university: incomingEducation?.university !== undefined ? incomingEducation.university : req.user.education?.university,
            graduationYear: incomingEducation?.graduationYear !== undefined ? Number(incomingEducation.graduationYear) : req.user.education?.graduationYear,
            degree: incomingEducation?.degree !== undefined ? incomingEducation.degree : req.user.education?.degree,
        };
        ...
        const updatedUser = await User.findByIdAndUpdate(
            req.user.id,
            {
                ...
                professionalHeadline: professionalHeadline !== undefined ? professionalHeadline : req.user.professionalHeadline,
...
exports.getProfileByUsername = async(req, res) => {
    try {
        ...
        const userProfile = await User.findOne({ username })
            .select('-password -emailVerificationOtp -otpExpires -resetPasswordToken -resetPasswordExpires');
        if (!userProfile) {
...
exports.verifyMobileOtp = async (req, res) => {
    try {
        ...
        if (user.isMobileVerified) return res.status(400).json({ message: 'Mobile number already verified' });
        if (!user.mobileNumber) return res.status(400).json({ message: 'Please add a mobile number first.' });
