...
exports.getEventSubmissions = async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin' && !isJudge) {
      return res
        .status(403)
        .json({ message: 'Access denied. You are not a judge for this event.' });
    }
...
exports.submitEvaluation = async (req, res) => {
  try {
    ...
    if (req.user.role !== 'admin' && !authorizedJudges.includes(judgeId)) {
      return res
        .status(403)
        .json({ message: 'Security Alert: You are not authorized to score this event.' });
    }
...
exports.publishLeaderboard = async (req, res) => {
  try {
    ...
    if (!isOrganizer && !isAdmin) {
      return res
        .status(403)
        .json({ message: 'You are not authorized to publish results for this event.' });
    }
    ...
    res.status(200).json({
      message:
        'Leaderboard published successfully! Status updated to Completed and notification emails have been dispatched.',
      eventStatus: event.status,
    });
  } catch (error) {
    res.status(500).json({ message: 'Server Error', error: error.message });
