...
exports.createTeam = async (req, res) => {
    try {
        ...
        if (!isRegistered) {
            return res.status(403).json({ message: "You must register for the event before creating a team." }); // Fixed typo
        }
...
exports.requestToJoin = async (req, res) => {
    try {
        ...
        if (!isRegistered) {
            return res.status(403).json({ message: "You must register for the event before joining a team." });
        }

        // 2. Capacity & Duplication Checks
        if (team.members.length >= 4) return res.status(400).json({ message: "This team is already full." });
        if (team.members.includes(userId)) return res.status(400).json({ message: "You are already a member of this team." });
        if (team.pendingRequests.includes(userId)) return res.status(400).json({ message: "You have already sent a request to this team." });
        ...
        res.status(200).json({ message: "Join request sent successfully! The leader has been notified." });
...
exports.inviteUserToTeam = async (req, res) => {
    try {
        ...
        const isRegistered = await Registration.findOne({ user: userId, event: team.event._id });
        if (!isRegistered) return res.status(400).json({ message: "This user is not registered for the event." });

        const isUserInTeam = await Team.findOne({ event: team.event._id, members: userId });
        if(isUserInTeam) return res.status(400).json({ message: "User has already joined another team." });
...
exports.acceptInvite = async (req, res) => {
    try {
        ...
        if (!team.invitedUsers.includes(userId)) {
            return res.status(403).json({ message: "You do not have a pending invitation to this team." });
        }
...
exports.leaveTeam = async(req, res) => {
    try {
        ...
        if (team.leader.toString() === userId) {
            return res.status(400).json({ message: "As the team captain, you cannot leave. You must disband the team or transfer leadership." });
        }
        ...
        if (now >= eventStartDate) {
            return res.status(403).json({ message: "The hackathon has already started. Roster is locked!" });
        }
...
exports.removeMember = async(req, res) => {
    try {
        ...
        if (userId === leaderId) {
            return res.status(400).json({ message: "You cannot remove yourself. Use the disband route instead." });
        }

        const eventEndDate = new Date(team.event.timing.endDate);
        if (new Date() > eventEndDate) {
            return res.status(403).json({ message: "The hackathon has ended. You can no longer modify the team." });
        }
...
exports.disbandTeam = async(req, res) => {
    try{
        ...
        const eventEndDate = new Date(team.event.timing.endDate);
        if (new Date() > eventEndDate) {
            return res.status(403).json({ message: "The hackathon has ended. Teams are locked for judging." });
        }

        await Team.findByIdAndDelete(teamId);

        res.status(200).json({ message: "Team has been permanently disbanded. All members are now free agents." }); // Fixed typo .sjon
    } catch (error) {
        res.status(500).json({ message: "Server Error", error: error.message });
    }
};
...
exports.submitProject = async (req, res) => {
    try {
        ...
        if (team.leader.toString() !== leaderId) {
            return res.status(403).json({ message: "Only the team captain can submit the final project." });
        }
...
exports.rejectInvite = async (req, res) => {
    try {
        ...
        if (!team.invitedUsers.includes(userId)) {
            return res.status(400).json({ message: "You do not have a pending invitation to this team." });
        }

        team.invitedUsers = team.invitedUsers.filter(id => id.toString() !== userId);
        await team.save();

        res.status(200).json({ success: true, message: `You have successfully declined the invitation from ${team.name}.` });
    } catch (error) {
        res.status(500).json({ message: "Server Error", error: error.message });
    }
};