...
exports.createEvent = async(req, res) => {
    try {
        ...
        if (phases) {
            if (phases.hackingStart && !eventTiming.startDate) eventTiming.startDate = phases.hackingStart;
        ...
        if (new Date(regDeadline) > new Date(eventTiming.endDate)) {
            return res.status(400).json({ message: "Registration deadline must be before the event ends." });
        }
...
exports.updateEvent = async (req, res) => {
    try {
        ...
        if (ticketing) {
            ...
            const total = ticketing.totalSeats !== undefined ? Number(ticketing.totalSeats) : event.ticketing.totalSeats;
...
exports.uploadEventBanner = async (req, res) => {
    try {
        ...
        // Authorization check
        if (event.organizer.toString() !== req.user.id && req.user.role !== 'admin') {
            return res.status(403).json({ message: "You are not authorized to upload banners for this event." });
        }
...
exports.getAssignedEvents = async (req, res) => {
    try {
        const events = await Event.find({ judges: req.user.id }).populate('organizer', 'firstName lastName email');
        res.status(200).json({
