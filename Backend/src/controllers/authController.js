  ...
  try {
    ...
    if (user.isBanned) {
      return res.status(403).json({
        message:
          'Your account has been suspended by an admininstrator for violating platform guidelines.',
      });
    }
...
exports.deleteAccount = async (req, res) => {
  try {
    ...
    if (ledTeams.length > 0) {
      return res.status(400).json({
        message:
          'You cannot delete your account while you are the captain of an active team. Please disband your team first.',
      });
    }
    await Team.updateMany({ members: userId }, { $pull: { members: userId } });
    await Team.updateMany({ pendingRequests: userId }, { $pull: { pendingRequests: userId } });
    await Team.updateMany({ invitedUsers: userId }, { $pull: { invitedUsers: userId } });
    ...
    res
      .status(200)
      .json({ message: 'Account and all associated data have been permanently deleted.' });
  } catch (error) {
    res.status(500).json({
      message: 'Server Error',
      error: error.message,
...
exports.refreshToken = async (req, res) => {
  try {
    ...
    if (!savedToken) {
      ...
      return res
        .status(401)
        .json({ message: 'Compromised session detected. Logged out of all devices.' });
    }
